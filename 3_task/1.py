...
def draw_ui():
    display_surface.blit(canvas, (0, 0))  # Отрисовка содержимого холста
...
while True:
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            if tool in [TOOL_PENCIL, TOOL_ERASER]:
                pygame.draw.circle(canvas, WHITE if tool == TOOL_ERASER else current_color, event.pos, brush_size)
